...
최초 접속 시 각 기능을 카드 형태로 소개하는 홈 화면이 먼저 보이며(카드 전체를 클릭하면 바로 해당 기능으로 이동), 사이드바는 "업무 도구 1 / 업무 도구 2 / 개인 관심사" 3개 그룹(그룹별 포인트 컬러로 구분)으로 나뉘어 있고 기본적으로 접혀 있습니다(화면 왼쪽 위 화살표로 펼칠 수 있음). 전역 색상 테마는 `.streamlit/config.toml`에서 관리합니다.
...
| 📐 문서스캔변환기 | 비스듬하게 찍힌 서류 사진에서 4개 꼭짓점을 클릭하면 반듯한 사각형 이미지로 원근 보정, png/jpg/pdf(자동 크기 또는 A4)로 다운로드 |
